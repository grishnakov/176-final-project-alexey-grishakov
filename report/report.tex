...
The motivation for this project is to test whether an attention-free architecture can retain strong language-modeling performance while reducing the training and long-context costs associated with standard self-attention. If a geometric mixer can match or exceed a Transformer at similar scale, it could offer a more efficient alternative for future long-context language models.
...
While the core project remains the same as in the proposal, the final evaluation setup differs from the original plan: instead of external benchmarks such as WikiText-103 and Lambada, I evaluate both models on a shared held-out split drawn from the same FineWeb-derived token stream used in training.
...
\begin{itemize}
    \item \textbf{Training data:} FineWeb-derived token stream consisting of a \(100\)B-token sample, downloaded using the recommended method on the dataset's Hugging Face page and then used in GPT-2-tokenized, pre-tokenized form \cite{penedo2024the}.
...
\begin{itemize}
    ...
    \item Benchmark logs,
    \item \href{https://youtu.be/XVWqi8_S1Xc}{Demo video}
